...
let%expect_test "Hashtbl.update_or_null" =
  let t = Hashtbl.create (module String) in
  let print () = print_s [%sexp (t : int Hashtbl.M(String).t)] in
  (* Insert a new key: Null -> v *)
  Hashtbl.update_or_null t "a" ~f:(function
    | Null -> 1
    | This _ -> failwith "expected missing key");
  print ();
  [%expect {| ((a 1)) |}];
  (* Update an existing key: This v1 -> v2 *)
  Hashtbl.update_or_null t "a" ~f:(function
    | This v -> v + 10
    | Null -> failwith "expected existing key");
  print ();
  [%expect {| ((a 11)) |}]
;;

let%expect_test "Hashtbl.update_or_null_and_return" =
  let t = Hashtbl.create (module String) in
  let update_or_null_and_return str ~f =
    let x = Hashtbl.update_or_null_and_return t str ~f in
    print_s [%message (t : (string, int) Hashtbl.t) (x : int)]
  in
  update_or_null_and_return "foo" ~f:(function
    | Null -> 1
    | This _ -> failwith "expected missing key");
  [%expect {| ((t ((foo 1))) (x 1)) |}];
  update_or_null_and_return "foo" ~f:(function
    | This 1 -> 2
    | Null | This _ -> failwith "expected existing key");
  [%expect {| ((t ((foo 2))) (x 2)) |}]
;;
