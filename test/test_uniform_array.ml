...
let%expect_test "fold_until" =
  ...
  let test a init f finish =
    let result = fold_until ~init ~f ~finish (of_list a) in
    print_s [%message (result : int)]
  in
  test [] () (fun _ _ -> failwith "don't call me") (fun _ -> 1);
  [%expect {| (result 1) |}];
  let a = [ 1; 2; 3; 5; 8 ] in
  test a () (fun _ _ -> Stop 1) (fun _ -> failwith "don't call me");
  [%expect {| (result 1) |}];
  test
    a
    0
    (fun x y -> if y < x + 3 then Continue x else Stop x)
    (fun _ -> failwith "don't call me");
  [%expect {| (result 0) |}];
  test a 0 (fun x y -> if y < x + 4 then Continue x else Stop x) (fun x -> x + 10);
  [%expect {| (result 0) |}]
;;
...
module%test [@tags "no-js"] _ : module type of struct
  ...
  let%template get = (Uniform_array.get [@mode c]) [@@mode c = (uncontended, shared)]
  ...
  let fold_until = Uniform_array.fold_until
