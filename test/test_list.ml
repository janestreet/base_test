...
let box_pair (a, b) = a, b

let%test_unit _ =
  [%test_result: int list * int list]
    (split_n [ 1; 2; 3; 4; 5; 6 ] 3 |> box_pair)
    ~expect:([ 1; 2; 3 ], [ 4; 5; 6 ])
;;

let%test_unit _ =
  [%test_result: int list * int list]
    (split_n [ 1; 2; 3; 4; 5; 6 ] 100 |> box_pair)
    ~expect:([ 1; 2; 3; 4; 5; 6 ], [])
;;

let%test_unit _ =
  [%test_result: int list * int list]
    (split_n [ 1; 2; 3; 4; 5; 6 ] 0 |> box_pair)
    ~expect:([], [ 1; 2; 3; 4; 5; 6 ])
;;

let%test_unit _ =
  [%test_result: int list * int list]
    (split_n [ 1; 2; 3; 4; 5; 6 ] (-5) |> box_pair)
    ~expect:([], [ 1; 2; 3; 4; 5; 6 ])
;;
...
let%expect_test "[merge_all]" =
  let test_int lists =
    let result = merge_all lists ~compare:Int.compare in
    print_s [%sexp (result : int list)];
    (* Sanity-check against a slow O(n*k) reference. *)
    let reference =
      List.fold lists ~init:[] ~f:(fun acc l -> merge acc l ~compare:Int.compare)
    in
    require_equal
      (module struct
        type t = int list [@@deriving equal, sexp_of]
  ...
  test_int [ [ 1; 3; 5 ] ];
  [%expect {| (1 3 5) |}];
  test_int [ [ 1; 4; 7 ]; [ 2; 5; 8 ]; [ 3; 6; 9 ] ];
  [%expect {| (1 2 3 4 5 6 7 8 9) |}];
  test_int [ [ 1; 2; 3 ]; []; [ 4; 5 ]; []; [ 6 ] ];
  [%expect {| (1 2 3 4 5 6) |}];
  test_int [ [ 1; 1 ]; [ 1 ]; [ 1; 1; 1 ] ];
  [%expect {| (1 1 1 1 1 1) |}];
  (* Ties: elements from earlier lists should come first. *)
  let test_pair lists =
    let result = merge_all lists ~compare:[%compare: int * _] in
    print_s [%sexp (result : (int * string) list)]
  in
  test_pair [ [ 1, "a"; 2, "a" ]; [ 1, "b"; 2, "b" ]; [ 1, "c"; 2, "c" ] ];
