...
module%test _ : module type of Avltree = struct
  ...
  include struct
    ...
    [@@kind k = k, v = v, r = (v, value_or_null)] [@@mode c = (uncontended, shared)]
    ;;]
    ...
    let findi_and_call = (findi_and_call [@kind k v v] [@mode c])
    [@@kind k v v] [@@mode c = (uncontended, shared)]
    ;;]
    ...
    let find_and_call1 = (find_and_call1 [@kind k v value_or_null v] [@mode c])
    [@@kind k v value_or_null v] [@@mode c = (uncontended, shared)]
    ;;]
    ...
    let findi_and_call1 = (findi_and_call1 [@kind k v value_or_null v] [@mode c])
    [@@kind k v value_or_null v] [@@mode c = (uncontended, shared)]
    ;;]
    ...
    let find_and_call2 =
      (find_and_call2 [@kind k v value_or_null value_or_null v] [@mode c])
    [@@kind k v value_or_null value_or_null v] [@@mode c = (uncontended, shared)]
    ;;]
    ...
    let findi_and_call2 =
      (findi_and_call2 [@kind k v value_or_null value_or_null v] [@mode c])
    [@@kind k v value_or_null value_or_null v] [@@mode c = (uncontended, shared)]
    ;;]
