...
module%test [@tags "64-bits-only"] _ : Impl = struct
  ...
  (* Finicky invariants, not covered. *)

  (* As of 2026-05, these three are implemented with [merge_by_case], which is covered. *)
  let merge_aligned = Map.merge_aligned
  let zip = Map.zip
  let zip_exn = Map.zip_exn
