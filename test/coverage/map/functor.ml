...
module Test_transformers
  ...
  and () =
    quickcheck_m (module Inst_pair) ~f:(fun t ->
      ...
      require_equal
        (module Data.Pair (Alist))
        (to_alist xs, to_alist ys)
        (to_alist t |> List.map ~f:(fun (key, (x, y)) -> (key, x), (key, y)) |> List.unzip);
      require_equal (module Inst_pair.Value) t (access zip_exn xs ys);
      require_equal (module Data.Or_error (Inst_pair.Value)) (Ok t) (access zip xs ys))
  ...
  and merge_aligned = merge_aligned

  and () =
    quickcheck_m (module Inst_and_inst) ~f:(fun (a, b) ->
      let a = Inst.value a in
      let b = Inst.value b in
      let f ~key x y = Key.to_int key + x + y in
      let actual = access merge_aligned a b ~f in
      let expect =
        match [%equal: Key.t list] (keys a) (keys b) with
        | false -> Or_error.error_string "oops"
        | true ->
          List.map2_exn (to_alist a) (to_alist b) ~f:(fun (key, a) (_, b) ->
            key, f ~key a b)
          |> create of_alist_exn
          |> Or_error.return
      in
  ...
  and () =
    quickcheck_m (module Inst_and_inst) ~f:(fun (a, b) ->
      let a = Inst.value a in
      let b = Inst.value b in
      let actual = access zip a b in
      let expect =
        match [%equal: Key.t list] (keys a) (keys b) with
        | false -> Or_error.error_string "oops"
        | true ->
          List.map2_exn (to_alist a) (to_alist b) ~f:(fun (key, a) (_, b) -> key, (a, b))
          |> create of_alist_exn
          |> Or_error.return
      in
      require_equal (module Data.Or_error (Inst_pair.Value)) actual expect)
  ...
  and () =
    quickcheck_m (module Inst_and_inst) ~f:(fun (a, b) ->
      let a = Inst.value a in
      let b = Inst.value b in
      let actual = Or_error.try_with (fun () -> access zip_exn a b) in
      let expect = access zip a b in
      require_equal (module Data.Or_error (Inst_pair.Value)) actual expect)
